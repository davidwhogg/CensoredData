...
p(\fobs |\sigma^2_i,\theta,I) &=& \left\{\begin{array}{ll}
  N(\ftrue | \mu_i,  \sigma^2_i + s^2_{\mu})\,  \int_{-\infty}^{\ftrue} p(b_i | \theta)\, \dd b_i & \mbox{if $\fobs \ne \texttt{NA}$} \\
...
\end{eqnarray}
where we have introduced the hyperparameters $B$ and $V_B$ for the Gaussian prior distribution of $b_i$.  In Equation \ref{eq:mlik}, in the epochs for which a detection was made ($\fobs \ne \texttt{NA}$), we marginalize over the unknown $b_i$ from $0$ (low brightness limit) to $\ftrue$, enforcing that the detection limit be fainter than the observed brightness.  In the epochs for which no detection was made ($\fobs = \texttt{NA}$), we compute the probability that the unknown detection threshold $b_i$ is greater than the unknown true flux $f_i$. When both $b_i$ and $f_i$ are normally distributed, this can be computed using a single call to a Gaussian cumulative distribution function, specifically
\begin{equation*}
\mathbb{P}(b_i \geq f_i) = \int_{-\infty}^0 N(x|B-\mu_i,V_B + \sigma_i^2 + s_{\mu}^2) dx
\end{equation*}
