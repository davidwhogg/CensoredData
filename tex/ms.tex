...
p(\fobs |\sigma^2_i,\theta,I) &=& \left\{\begin{array}{ll}
  N(\fobs | \mu_i,  \sigma^2_i + s^2_{\mu})\,  \int_{-\infty}^{\fobs} p(b_i | \theta)\, \dd b_i & \mbox{if $\fobs \ne \texttt{NA}$} \\
  \int_{-\infty}^{\infty} \int_{-\infty}^{b_i} N(\ftrue | \mu_i, \sigma^2_i + s^2_{\mu})\, p(b_i | \theta)\, \dd \ftrue\, \dd b_i & \mbox{if $\fobs = \texttt{NA}$} \\
...
%% \begin{equation*}
%% \mathbb{P}(b_i \geq f_i) = \int_{-\infty}^0 N(x|\mu_i - B,V_B + \sigma_i^2 + s_{\mu}^2) dx
%% \end{equation*}
...
p(\fobs |\sigma^2_i,\theta,I, b_i) &=& \left\{\begin{array}{ll}
 ...
 \int_{-\infty}^{b_i} N(\ftrue | \mu_i, \sigma^2_i + s^2_{\mu})\, \dd \ftrue & \mbox{if $\fobs = \texttt{NA}$} \\
...
all the epochs, as delivered by the survey) given the $2K + 5$ parameter vector $\theta$.
